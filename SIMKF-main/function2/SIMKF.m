...
DD = zeros(1, size(KH,3));
for i = 1:size(KH,3)
    temp = (eps + KH(:,:,i)) .* (eps + S0_initial);
    DD(i) = mean(temp(:));
end
alphaK = calculate_weights(DD , delta);
...
% --- Helper function for kernel weights ---
function alphaK = calculate_weights(DD, delta)
weights = exp(-DD / delta);
